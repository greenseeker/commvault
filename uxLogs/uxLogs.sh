...
# Updated 2022-06-03
# See https://connect.commvault.com/docs/DOC-3728 for latest version
...
if (contains "$params" "--help"); then
 printf "\n%s\n\n" "This is a Commvault log collection script for AIX, FreeBSD, HPUX, Linux, Mac, and Solaris."
 printf "%s\n" "It captures the Commvault log directories for each instance as well as:"
 printf "%s\n" " Basic system info (uname -a)"
 printf "%s\n" " commvault list output (or oem equivalent)"
 printf "%s\n" " Commvault's network certificates directory"
 printf "%s\n" " Filesystem stats (df, mount)"
 printf "%s\n" " OS messages log, syslog, system log or Error Report"
 printf "%s\n" " Firewall state (nft, firewalld, iptables, ipfw, ipf)"
 printf "%s\n" " SELinux state"
 printf "%s\n" " Kernel parameters"
 printf "%s\n" " Network interface information (ip addr/ifconfig/netstat)"
 printf "%s\n" " Commvault Registry (/etc/CommVaultRegistry)"
 printf "%s\n" " Hosts file"
 ...
 printf "%s\n" " HyperScale-related logging on applicable systems"
...
[ -z "$JID" ] && tarball=$tmpDir/${hostname}_uxLogs.tar || tarball=$tmpDir/${hostname}_${JID}_uxLogs.tar
...
case ${flavor} in
 Linux) 
  ...
  which firewall-cmd >> /dev/null 2>&1 && echo "=== firewalld ===" >> $tmpDir/cvcollect/firewall.txt && firewall-cmd --list-all-zones >> $tmpDir/cvcollect/firewall.txt 2> /dev/null && printf "%s\n" "... firewalld rules collected" | tee -a $scriptLog
...
tar -rhf $tarball /var/log/.gxsetup /tmp/.gxsetup >> $scriptLog 2>&1 || printf "    %s\n\n" "--- Could not find any install logs; continuing ..."
